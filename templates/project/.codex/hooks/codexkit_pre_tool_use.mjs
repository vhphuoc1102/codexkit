...
import { findReservationConflicts } from "../codexkit_reservations.mjs";
import { resolveRepoRoot } from "../codexkit_state.mjs";
...
function inferAgentName(payload, toolInput) {
  const candidates = [
    ...
    getNestedString(toolInput, ["env", "CODEXKIT_AGENT_NAME"]),
  ];
...
function looksLikePathToken(token, repoRoot) {
  if (!token || token === "-" || ENV_ASSIGNMENT.test(token) || token.startsWith("-")) return false;
...
function extractCommandPaths(command, repoRoot) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (looksLikePathToken(token, repoRoot)) paths.push(token);
  }
  return [...new Set(paths)];
}

function isWriteHeavy(command) {
  if (command.includes(".codex/codexkit_reservations.mjs")) return false;
  if (/>|>>/.test(command)) return true;
  return WRITE_COMMANDS.some((pattern) => pattern.test(command));
}

function renderConflictSummary(conflicts) {
  return conflicts.map((conflict) => `${conflict.agent} -> ${conflict.paths.join(", ")}`).join(" ; ");
}
...
export async function main() {
  ...
  if (BARE_BV.test(command) && !command.includes("--robot-")) {
    message = "CodexKit expects `bv` only with `--robot-*` flags in agent sessions. Bare `bv` launches the interactive TUI.";
  }
  ...
  if (!message && isWriteHeavy(command)) {
    ...
    if (conflicts.length > 0) {
      ...
      if (agentName) {
        continueRun = false;
        message =
          `CodexKit local reservations blocked this shell command for ${agentName}. ` +
          `Requested paths: ${pathsToCheck.join(", ")}. Active conflicts: ${conflictSummary}.`;
      } else {
        message =
          "CodexKit local reservations detected overlapping active claims, but this hook could not " +
          "determine the current agent identity from the Bash payload so it is warning instead of blocking. " +
          `Requested paths: ${pathsToCheck.join(", ")}. Active conflicts: ${conflictSummary}. ` +
          "Prefix write-heavy shell commands with `CODEXKIT_AGENT_NAME=<codex nickname>` to enable ownership-aware blocking.";
      }
    }
  }
